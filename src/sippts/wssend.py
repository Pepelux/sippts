...
class WsSend:
    ...
    def start(self):
        ...
        if self.method not in supported_methods:
            print(self.c.RED + 'Method %s is not supported' %
                  self.method + self.c.WHITE)
            sys.exit()

        # check protocol
        if self.proto not in supported_protos:
            print(self.c.RED + 'Protocol %s is not supported' %
        ...
        self.msg = create_message(self.method, local_ip, self.contact_domain, self.from_user, self.from_name, self.from_domain, self.to_user, self.to_name, self.to_domain, self.proto,
                                  self.domain, self.user_agent, lport, '', '', self.from_tag, '1', self.to_tag, '', 1, '', 0, '', '', self.ppi, self.pai, '', 1)
        ...
        try:
            ...
            print(self.c.RED + 'Socket error' + self.c.WHITE)
            exit()
        ...
        wss = websocket.WebSocketApp("wss://%s:%s%s" % (self.ip, self.rport, self.path),
